define([], function(){
    ...
    */
	...
	function drawBall(layer,ball,board,color,game){
        ...
        circ.boardball =;
        circ.on('dragmove', function(){
            this.ball.x = (this.x()/layer.width()*board.width)+board.x;
            this.ball.y = -(this.y()/layer.height()*board.height)+board.height;
            //console.log("dragging rectangle to "+goal.x+","+goal.y);
        });
	    layer.add(circ);
	    return circ
	}
	...
	function drawBalls(layer,balls,board,color,game){
	    for (var i=0; i<balls.length;i++){
		drawBall(layer,balls[i],board,color,game);
	    }
	}
        
    function drawRect2(layer,goal,board,color){
	    //console.log(goal);
	...
	function drawRect(layer,goal,board,color){
        ...
        rect.on('dragmove', function(){
            goal.x = (this.x()/layer.width()*board.width)+board.x;
            goal.y = -(this.y()/layer.height()*board.height)+board.height;
            //console.log("dragging rectangle to "+goal.x+","+goal.y);
        });
	    layer.add(rect);
	}
...
//	console.log(game);
	...
	drawBalls(balllayer, game.gameballs, game.challenge.board, "#FF0000",game.challenge);
	
	drawRect(layer, game.challenge.goal, 
		 game.challenge.board, "#00FF00",game.challenge);
	drawObstacles(layer, game.challenge.obstacles,
		      game.challenge.board, "#882200",game.challenge);
	stage.add(layer);
	stage.add(balllayer);
	stage.draw();
